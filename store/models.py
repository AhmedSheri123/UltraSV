...
class Services(models.Model):
    ...
    storeTypes = models.ForeignKey(StoreTypes, on_delete=models.CASCADE, verbose_name="نوع المتجر", null=True, blank=True)
    service_Type = models.ManyToManyField(ServiceTypes, verbose_name="نوع الخدمة", blank=True)
    keyword = models.TextField(verbose_name="كلمات مفتاحية", null=True, blank=True)
    is_from_url = models.BooleanField(default=False, verbose_name="هل رابط تحميل الخدمة من الخارج")
    service_url = models.CharField(max_length=255, verbose_name="رابط الخدمة", blank=True)
    ...
    download_btn_get_without_getDownloadsLink = models.CharField(max_length=255,  verbose_name="اذهب الى قطعة من الصفحة", blank=True)
    service_file = models.FileField(upload_to="service/files/%Y/%m/", verbose_name='رفع ملف', blank=True)
    ...
    file_size_format = models.CharField(max_length=255, choices=download_size_type_list, verbose_name="صغة حجم الملف المراد تنزيله", null=True,  blank=True)
    issued = models.DateField(null=True, verbose_name="تاريخ الاصلي التي  نشرت لاول مرة على الانترنت",  blank=True)
...
class EditedServicesWaitForAccept(models.Model):
    service = models.ForeignKey(Services, on_delete=models.CASCADE, verbose_name='الخدمة المعدلة')
    user = models.ForeignKey(User, on_delete=models.SET_NULL, verbose_name='الناشر', null=True, blank=True)
    title = models.CharField(max_length=255, verbose_name="عنوان المقالة", blank=True)
    desc = models.TextField(verbose_name="وصف", null=True, blank=True)
    article_img = models.ImageField(upload_to='article/img/%Y/%m/%d', null=True, blank=True)
...
class WaitForAccept(models.Model):
    service = models.ForeignKey(Services, on_delete=models.CASCADE, verbose_name='الخدمة')
